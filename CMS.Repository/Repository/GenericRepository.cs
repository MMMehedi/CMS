...
namespace CMS.Repository.Repository
{
    public class GenericRepository<TEntity> : IRepository<TEntity> where TEntity : class
    {
        ...
        public virtual async Task<IEnumerable<TEntity>> FirstOrDefault(Expression<Func<TEntity, bool>> predicate)
        {
            try
            {
        ...
        public async Task<dynamic> Save(int option)
        {
            try
            {
                ...
                if (res == 1 && option == 3)
                {
                    return "Data Accepted Successfullly";
                }
            }
            catch (Exception ex)
            {
        ...
        public Task FindBy(Func<dynamic, bool> p)
        {
